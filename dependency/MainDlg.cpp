...
//  TVIF_PARAM	
// 	int length = wcslen(pszCaption);
// 	wchar_t *text_buffer = (wchar_t *)malloc((length + 1)*sizeof(wchar_t));
// 	memcpy(text_buffer, pszCaption, length * 2);
// 	text_buffer[length] = '\0';
	tree_view_item.pszText = (LPTSTR)pszCaption;
	tree_view_item.cchTextMax = wcslen(pszCaption);
//	tree_view_item.lParam = (LPARAM)text_buffer;
	...
	// 	if(nLevel > ){
	// 		hti = TreeView_GetParent(tree_view_, hPrev);
...
void CMainDlg::AddListViewItem(int item_index, int column_index, LPCTSTR text)
{
	...
	// ListView_InsertItem(list_view_, &list_view_item);
...
void CMainDlg::AddExportFunctions(const std::list<IMAGE_EXPORT_FUNCTION>& function_list)
{
	...
	for (auto& function : function_list) {
		...
		wchar_t buffer[16] = { 0 };
		swprintf(buffer, L"%2d(0x%04x)", function.ordinal, function.ordinal);
		AddListViewItem(count, 0, buffer);
		swprintf(buffer, L"%2d(0x%04x)", function.hint, function.hint);
