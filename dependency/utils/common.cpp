...
std::string UToA(const std::wstring& str)
{
    ...
    if (str.empty())
        goto __end;
    int nLen = ::WideCharToMultiByte(CP_ACP, 0, str.c_str(), (int)str.size(), NULL, 0, NULL, NULL);
    if (0 == nLen)
        goto __end;
    char* pBuffer = new char[nLen + 1];
    memset(pBuffer, 0, nLen + 1);
    ::WideCharToMultiByte(CP_ACP, 0, str.c_str(), (int)str.size(), pBuffer, nLen, NULL, NULL);
    pBuffer[nLen] = '\0';
